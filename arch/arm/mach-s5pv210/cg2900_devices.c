...
//#include <mach/gpio.h> removed as gpio-aries.h has removed it
#include <mach/gpio-aries.h>

//++Adding gpio control for power enable 
/* ++Hemant: ESD Clamp Timing Update */
#define PDB_INITIAL_HIGH_TIMEOUT  (130)
#define PDB_INITIAL_LOW_TIMEOUT   (10) 
#define VDD_1_8_V_HIGH_TIME	   (200)	
/* --Hemant: ESD Clamp Timing Update */

#define GBF_ENA_PWR_EN_GPIO   GPIO_GPS_PWR_EN
#define GBF_ENA_POWER_NAME            "gbf_power_ena"
//--Adding gpio control for power enable 
...
//++Adding gpio control for power enable 
	...
	printk(KERN_DEBUG "cg2900_devices_enable_chip-PDB*PWR");

	gpio_set_value(GBF_ENA_PWR_EN_GPIO, 1);   

/* ++Hemant: ESD Clamp Timing Update */
	udelay(VDD_1_8_V_HIGH_TIME);   
/* --Hemant: ESD Clamp Timing Update */

       gpio_set_value(GBF_ENA_RESET_GPIO, 1);

/* ++Hemant: ESD Clamp Timing Update */
	udelay(PDB_INITIAL_HIGH_TIMEOUT);
/* --Hemant: ESD Clamp Timing Update */

       gpio_set_value(GBF_ENA_RESET_GPIO, 0);

/* ++Hemant: ESD Clamp Timing Update */
	udelay(PDB_INITIAL_LOW_TIMEOUT);  
/* --Hemant: ESD Clamp Timing Update */
...
{
	printk(KERN_DEBUG "cg2900_devices_disable_chip-PDB*PWR");

	gpio_set_value(GBF_ENA_RESET_GPIO, 0);

//++Adding gpio control for power enable 
	gpio_set_value(GBF_ENA_PWR_EN_GPIO, 0);
 //--Adding gpio control for power enable 
...
struct sk_buff *cg2900_devices_get_power_switch_off_cmd(u8 *op_lsb, u8 *op_msb)
{
	...
	/* ++Hemant: GPIO values during Switch Off State of CG2900 */ 
	/* ++Hemant: Update UART GPIO's as Pull None */ 
	data[3] = 0x00; /* GPIO 0-7 pull-up (All GPIO's No Pull Up) */
	/* --Hemant: Update UART GPIO's as Pull None */ 
	data[4] = 0x00; /* GPIO 8-15 pull-up (All GPIO's No Pull Up) */
	data[5] = 0x00; /* GPIO 16-20 pull-up (All GPIO's No Pull Up) */
	data[6] = 0xC3; /* GPIO 0-7 pull-down (All GPIO's expect  2-5 set to Pull Down)*/
	data[7] = 0xFF; /* GPIO 8-15 pull-down (All GPIO's Pull Down)*/
	data[8] = 0x1F; /* GPIO 16-20 pull-down (All GPIO's Pull Down)*/
	/* --Hemant: GPIO values during Switch Off State of CG2900 */ 
